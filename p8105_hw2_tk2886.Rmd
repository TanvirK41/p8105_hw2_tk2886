...
combined_median = precipitation_data = precipitation_data %>% summarise(median(total))
```
...
```
After importing and cleaning the **Mr. Trash Wheel** data . The important variables names are **`r variable_trashwheel`**. The *total rows* in Mr.Trash Wheel data is **`r row_trashwheel`**. The *total columns* in Mr.Trash Wheel data is **`r col_trashwheel`**. The *median number of sports ball* is **`r median_sportsball`**. The *mean number of sports ball* is **`r mean_sportsball`**.
...
```
The *"pols-month"* data set contains data that have national politicians who are democratic or republican. The important variables in the pols-month data set are **`r name_poldata`**. The *dimensions* of the pols-month data is **`r dim_poldata`**. The *range of years* in the pols-month data is from **`r range_year`**. 
