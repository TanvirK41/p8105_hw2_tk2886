...
```
After importing and cleaning the **2018 precipitation** data . There are important information that may be examined. 
The *total rows* in precipitation data in 2018 is **`r p18_row`**. The *total columns* in precipitation data in 2018 is **`r p18_col`**. The *important variables* in the precipitation data in 2018 are **`r variable_p18`**.The *total precipitation* in 2018 was **`r total_precipitation18`**. The *median precipitation* in 2018 was **`r median_p18`**. The *mean precipitation* in 2018 was **`r mean_p18`**.

After importing the **2019 precipitation** data. There are important information that may be examined. The *total rows* in precipitation data in 2019 is **`r p19_row`**. The *total columns* in precipitation data in 2018 is **`r p19_col`**. The *important variables* in the precipitation data in 2019 are **`r variable_p19`**. The *total precipitation* in 2019 was **`r total_precipitation19`**. The *median precipitation* in 2019 was **`r median_p19`**. The *mean precipitation* in 2019 was **`r mean_p19`**. 



### Analysis of Mr. Trash Wheel Data
```{r}
variable_trashw = trashwheel_data %>% names()
row_trashwheel = trashwheel_data %>% nrow()
col_trashwheel = trashwheel_data %>% ncol()
median_sportsball = trashwheel_data %>% summarise(median(sports_balls))
```
After importing and cleaning the **Mr. Trash Wheel** data . There are important information that may be examined. The *total rows* in Mr.Trash Wheel data is **`r row_trashwheel`**. The *total columns* in Mr.Trash Wheel data is **`r col_trashwheel`**. The *median number of sports ball* is **`r median_sportsball`**.
