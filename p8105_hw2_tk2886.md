...
stock market as a whole. The *important variables* in the snp dataset
...
uneployment data set are **816, 3**. The *range of years* in the
...
``` r
male_wnh16 = ggplot(scatterplot_data, aes(x = rank, y = count, color = childs_first_name)) +
  geom_point() + 
